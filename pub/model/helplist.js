...
const roles = {
    async add(args) {
        let sql =
            "INSERT INTO helplist (wx_id,title,state,des,a_id,total_fee,form_id,mu,qi,file,cai,order_num,openid,page, is_pay) value(?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?)";
        let params = [
            args.wx_id,
            args.title,
            args.state,
    ...
    async updatePayed(args) {
        let sql =
            "UPDATE helplist set state=1,pay_time=now(),is_pay=1 where id = ?";
        let params = [args.id];
        let result = await db.query(sql, params);
        return result;
    },
    ...
    async jd(args) {
        let sql =
            "update helplist set state = 2,jd_id=?,jd_time=now() where id = ?";
