...
const roles = {
    ...
    async updateJd(ctx) {
        ...
        if (bkdata.errno) {
            ...
            token.msgModel(
                form.openid,
                "8STlOtBZ4TrivsA8uD_5_SIGjYzWJve29Jm4wd3Q2Sk",
                form.form_id,
                "/pages/order/detail/detail?id=" + form.id,
                {
                    keyword1: {
    ...
    async getList(ctx) {
        ctx.request.body.fields =
            "helplist.*,wxuser.avatar_url,wxuser.nick_name ";
        ctx.request.body.tables = "helplist,wxuser";
        ctx.request.body.wheres =
            " helplist.wx_id = wxuser.id and helplist.is_delete = 0 and helplist.state in (1,2,3) and helplist.title in ('校园跑腿','上门维修','代替服务','其他帮助') and helplist.a_id=" +
            ctx.request.body.a_id;
