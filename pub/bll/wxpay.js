...
const { data } = require("../../../server_web/src/pages/wxuser/list/list.js");
...
const wxpay = {
    async wxpay(ctx, type) {
        let user_data = {
            ...
            package: Math.round(new Date().getTime() / 1000).toString(),
            paySign: "",
            ...
            qi: ctx.request.body.qi,
            file: ctx.request.body?.file || null,
            cai: ctx.request.body?.cai || null,
            order_num: `${new Date().getDate()}`,
            page: 0,
            is_pay: 1,
        };
        console.log(user_data);
        await model.add(user_data);
        return user_data;
        ...
        let bodyData = await this.bodyData(
            body,
            nonce_str,
            openid,
            out_trade_no,
        ...
        if (typeof cbdata == "string") {
            ...
            if (result.xml.return_code[0] == "SUCCESS") {
                ...
                if (type == "add") {
                    ...
                    addorder = await model.updateOrderNum(
                        out_trade_no,
                        param.oid
                    );
    ...
    async bodyData(
        ...
        bodyData +=
            "<spbill_create_ip>" + spbill_create_ip + "</spbill_create_ip>"; // 终端IP
        bodyData += "<total_fee>" + total_fee + "</total_fee>"; // 总金额 单位为分
        bodyData += "<trade_type>JSAPI</trade_type>"; // 交易类型 小程序取值如下：JSAPI
        // 签名
        var sign = await this.paysignjsapi(
            config.APP_ID,
            body,
            config.Mch_id,
            nonce_str,
